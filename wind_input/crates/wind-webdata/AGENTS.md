...
## 关键约束
- **依赖方向：本 crate → wind-coordinator**，绝不能反向。coordinator 不依赖本 crate 的 wind-transfer/fontdb，Android 闭包（wind-android 不依赖本 crate）因此无任何 C 依赖，`cargo check-android` 本机免 NDK 可跑。
