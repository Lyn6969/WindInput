...
pub trait Engine: Send + Sync {
    ...
    /// 本引擎候选的**最大 weight**——词频等效权重的量纲基准
    /// （`docs/design/freq-weight-model.md` §7）。默认 `None` = 不参与该模型。
    ///
    /// **必须由引擎给出而非配置常数**：混输把拼音候选整体 `/= PINYIN_TIER_SCALE`，
    /// 同一个词两种模式差 100 倍（`的` 15,378,475 vs 153,784，p50 甚至被整数除法归零），
    /// 且「码不全走双路线 / 超码长走拼音」的切换发生在**同一次输入过程中**，配置项跟不上。
    /// 量纲只有引擎自己知道，故基准在这一层给出。
