...
pub fn rerank_pinyin_positional(
    ...
    rerank_positional(
        ...
        // 消费长度 → 匹配层级（简拼 / 前缀补全 / 子短语）。词频不得跨层提拔。
        //
        // ## ⚠️ `cmp_by_consumed` 必须在前，且不能省
        //
        // 本函数是**最后一道整体排序**，会推翻 `candidate_display_order` 的结果；而它只在
        // 本次输入**有词频记录**时才被调用（`recs.is_empty()` 直接 return）。两处排序键不
        // 一致 ⇒ 同一批候选存在两种次序，且切换开关是「这个词有没有被上过屏」——用户看到的
        // 是候选忽隐忽现。实测：「冰冻三尺非一日之寒」打 `bingdongsanchi`（整音节边界，
        // 拿不到残码上浮 ⇒ 停在前缀补全层）首次在第 1 位，上屏一次后掉到第 24 位，
        // 从词频表删除又恢复。判据与完整事故记录见 `wind_candidate::cmp_by_consumed`。
        //
    ...
    L: Fn(&Candidate, &Candidate) -> std::cmp::Ordering,
{
    ...
    let meta: Vec<(bool, usize, f64)> = candidates
        ...
        .enumerate()
        .map(|(pos, c)| {
            let power = promotion_power(c, recs, now, profile, promote_prefix);
...
mod tests {
    ...
    /// `promote_prefix = false`（拼音默认）：前缀补全候选不参与位置提升。
    ///
    /// 真机现场（trace 日志）：打 `d` 时「东西」（`dongxi`，码比输入长）连选 6 次即从
    /// 第 40 位爬到首位，压过「的」「得」等高频单字。短输入下用户给出的信息量很少，
    /// 把长词组顶到单字前面与直觉相悖——微软拼音实测「只对全码生效」亦然。
    #[test]
    fn prefix_completion_not_promoted_when_switch_off() {
        let completion = |t: &str, w: i32| {
            let mut c = pin(t, w);
            c.is_prefix = true;
            c
        };
        // ⚠️ 打 `d` 时**所有候选都是前缀补全**（`de`/`dongxi` 的码都比 `d` 长），
        // 层级相同，位次才说了算——「东西」也才爬得上来。若把「的」「得」构造成精确匹配，
        // `cmp_match_layers` 会先把「东西」降层，本用例就测不到开关本身了。
        let build = || {
            vec![
                completion("的", W_DE),
                completion("得", W_DEI),
    ...
    /// 开关只挡**有效前缀层**：被引擎提升进完整匹配层的候选（残码上浮 / 用户长词上浮）
    /// 仍可调频——那是结构决策，与「码更长」无关。
    #[test]
    fn promoted_completion_still_reranks_when_switch_off() {
        let mut promoted = pin("没有", 5000);
        promoted.is_prefix = true;
