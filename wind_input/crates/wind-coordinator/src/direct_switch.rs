...
//! 2. 清理旧条目按 CLSID **或** Profile 匹配（Go 只匹配 CLSID，值缺失/损坏的自家
//!    旧条目永不清理，会残留脏数据）；
...
/// 当前构建变体的 (CLSID, guidProfile) 字符串（大写带花括号，与 wind_tsf Globals.cpp 一致）。
fn tip_guid_strings() -> (&'static str, &'static str) {
...
fn sync_inner(hotkey_desc: &str, entry: Option<(u32, u32)>) -> std::io::Result<()> {
    let (clsid, profile) = tip_guid_strings();
    ...
    for name in names {
        ...
        drop(sub);
        if c.eq_ignore_ascii_case(clsid) || p.eq_ignore_ascii_case(profile) {
