...
### 5.3 方案 c 已落地（`fa79ee1`，wdat v5）—— 附一个未预料的失效模式

**二进制结构完全没变**，变的只是 AbbrevSection 条目里那个字符串字段的语义
（词 → 全拼码），故必须 bump 版本：旧缓存按新逻辑读会把词当码拿去查主表、简拼全数落空。

> ⚠️ **同码不同切分**：扁平码有损，`xian` 既是「西安」的 `xi|an`（2 音节）也是
> 「先」的 `xian`（1 音节）。索引里 `xa` 指向前者的码，**回查主表却把后者一并捞出来**
> ——实测真实词库下 `xa` 出「先/线/弦/现/县」一串单字，且它们权重高得多、排在最前。
>
> 这是「存词 → 存码」引入的新失效模式，存词时不会发生（词是直接指定的，不经码这一跳）。
> 修法：按「简拼字母数 == 音节数」过滤（`boundary.count_ones()`），`boundary=0` 放行。
> **正好用上了同一次改动白送的 boundary。**
>
> 教训与 §2 同源：**凡把 flat 码当中转站的地方，都要问一句「这个码会不会对应多种切分」。**

另一处需要照顾的是聚合去重：同一简拼下多个词常共用一个全拼码（`nh` 的「你好」「拟好」
同为 `nihao`），存词时是不同条目、存码后成了重复项，故 `agg_ab` 内层改 map 取 max weight，
否则索引被撑大且截断时会挤掉别的码。
...
**系统词侧**已完成（`fa79ee1`，wdat v5）：`AbbrevSection` 改存 `abbrev → flat_code`，
简拼查询变成「查索引拿码 → 走主表装配」。详见 §5.3，含一个未预料的失效模式
（同码不同切分 ⇒ `xa` 捞出「先/线/弦」一串单字）。

### 附：词频列表的显示 ✅ **已完成**（`03a8e86`）—— 走「读时反查」

`FreqRecord` 只有 `count + last_used`，词频表是唯一不带 boundary 的持久层（§3.2），
边界只能反查。新增 `Engine::syllable_boundary_of(code, text)`：拿现成的码去词典
**点查取真值**。

> 与 `generate_word_pinyin` 的分水岭是**不做推断**。后者从词反推读音、多音字靠权重
> 消歧，可能给出与目标条目不同的码；而 `xian` 一个码下同时挂着「西安」(xi\|an) 与
> 「先」(xian)，只有按 `(code, text)` 精确定位才给得对。两者看着重复，实则不可互相取代。

反查三处依次问：**系统词典**（mmap 点查，最快也最可能命中）→ 用户词表 → 临时词表，
都查不到即保持扁平（存量简拼码记录、码表方案、词条已删除的遗留记录）。
只对**当前页**（≤ limit 条）反查，开销与词频表总规模无关。

**为何不给词频表扩容加字段**：词频是长期积累的数据，扩容只能让此后新写入的记录带
边界，用户会看到「新词有空格、老词没有」的混杂列表——而老词恰恰是词频表里最有价值的
部分。反查让存量立即受益，也不动存储格式。

**押后到 Phase 4 之后是对的**：词频表的 code 是候选的 code，简拼候选的 code 此前被
覆盖成简拼串，表里混着两种键；先把键清干净再补显示，否则边界会挂到简拼码上。

`freq.delete` 同步拆空格——这是同一个坑的**第四处**（用户词库 / 临时词库 / 搜索 /
词频），形态完全一致：**显示改了、回流入口没跟上**。

### Phase 5 — 三域结构统一（前两项 ✅ **已完成**，`6dd3cf3`）

- ✅ `ConvertedSyllable` 提升为通用 `SylSpan`（`sp_*` → `raw_*`），落在新模块
  `pinyin::interp`；`shuangpin` 侧 `pub use` 出去，`SpConvertResult` 字段类型不变
- ✅ `map_consumed_over_separators`（逐字节扫描）已由 `interp::map_fp_to_raw` 取代
- ⬜ 模糊变体候选校验边界（§11 遗留项）—— **行为变更**，会开始拒候选，需单独立项
- ⬜ `generate.rs` 的 410 音节暴力反推 —— 会改「代表读音 = 单字最高权重」语义，需单独确认

#### 实施中确认的两条事实

**残码必须在两域同步跳过。** 段内切分未覆盖的尾部（`anx` 只切出 `an`），若只推进
raw 不推进 flat，`xi'anx'hao` 里第三段的位置会整体错位。由此得到一条不变量：
**span 的 flat 总长恒等于剥除 `'` 后的串长**，映射因此永不越界（仍留防御性续推，
并写了测试锁住该不变量）。

**收尾规则不可丢**：紧跟的分隔符归入已消费侧（连续 `''` 一并吸收）。`xi'an` 选「西」
消费 flat 2 字节，若只返回 2，协调器按原始缓冲切片后剩下 `'an`，预编辑区会以分隔符
开头。旧函数的 8 个用例原样搬进 `interp` 作等价性判据，含 `nih'ao`（段内残码）、
`ni''hao`、`ni'` 三种边缘形态。

顺带消除一次重复切分：`convert` 的 `has_sep` 分支此前先取音节文本、再逐字节补算偏移，
现在一次 `spans_with_separators` 两样都有。

#### 评测确认为纯重构（真实词库，seed=20260721）
...
五项正确性指标与改动前**逐位相同**，仅耗时列正常波动。
**改热路径必须这样对账**——单元测试全绿不足以证明重构无回归。
