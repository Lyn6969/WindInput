...
| 五a | ~~A 类补全~~ ✅ | 已完成 |
| 五b | 冲突检测（方案级 × `leading_chars`） | 中，需 core 在 `schema.getConfig` 多返回一个字段 |
...
**五b 冲突检测**：真正有价值的那条是 §4.3 的**同层冲突**——方案把 `;` 绑成功能键，
而同一方案的 `leading_chars` 也含 `;`，内核裁决是「绑定优先」，于是码表的 `;` 起头
静默失效。要在设置页提示，需 core 在 `schema.getConfig` 里多返回该方案的
`leading_chars`（现在没有，见 `schema_codetable.rs` 的字段说明）。

方案级 × 全局 `trigger_keys` 的争用**不该报**：内核裁决明确（方案表优先跳过全局链），
且那正是这个功能的设计意图，报出来是噪音。

**五c 全局层收编**：把五处 `trigger_keys` 折算进 `keys.key_actions`、删掉硬编码优先级
链。这是唯一会**改所有用户既有行为**的一步（前四期都是「未配置者行为逐字节不变」），
需要配置迁移 + 设置页把五个触发键设置项合并成一张表 + 真机回归。单独排期。
