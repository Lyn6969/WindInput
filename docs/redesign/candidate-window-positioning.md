...
- **`fast` 档在 Excel「进应用后第一次输入」会漂移**（未解决，已知取舍）：Excel 首次输入要先切
  焦点、建立单元格编辑上下文（`sameDoc=0`），实测耗时 **454ms**，真坐标 558ms 才到。`wait` 档
  接受 `caret_pending` 的 600ms 延长，正好覆盖得住；而 **`fast` 档刻意拒绝该延长**（否则它在
  不发 `OnLayoutChange` 的宿主上会退化成 `wait`），只等 25ms，于是用按键前的旧坐标首显。
  之后的输入 context 已在，坐标立即可得，故只有第一次漂。
  > **`wait` 档在这个场景下不是靠判据赢的，是碰巧被 600ms 兜住了**——Excel 若再慢 50ms 它一样漂。
  >
  > 三条出路各有代价：① `focus_gained` 时清 `first_show_was_provisional`，使随后的权威坐标必然
  > 纠正（明确跳一下，但不会停在错位置）；② `focus_gained` 时重新 arm 首显（不先显示在错位置，
  > 但候选窗晚半秒出现——等于对「错位比迟钝更糟」这个原始取舍反转）；③ Excel 用 `wait`，`fast`
  > 只给验证过的宿主开（零代码）。**2026-08-02 用户拍板取 ③**。
